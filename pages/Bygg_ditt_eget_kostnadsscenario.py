...
def main():
    ...
    with st.container():
        ...
        interest_rate_range = st.slider(variables["INTEREST_RATE_SLIDER_LABEL"],
                                        min_value=variables["INTEREST_RATE_MIN"], max_value=variables["INTEREST_RATE_MAX"], value=tuple(variables["INTEREST_RATE_DEFAULT"]), step=0.25)
    ...
    with st.container():
        ...
        kwh_usage_range = st.slider(
                "Velg strømforbruk (kWh) per måned:",
                min_value=100,
                max_value=10000,
                value=500,
                step=100,
        ...
        fixed_cost_electricity = st.number_input("Fastprisledd (kr):", value=39, key='fixed_cost_nok')
